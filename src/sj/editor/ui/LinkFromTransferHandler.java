 ...
 * @author SafariJohn
 */
public class LinkFromTransferHandler extends TransferHandler {
    private static final Logger logger = Logger.getLogger(LinkFromTransferHandler.class.getName());

    DataFlavor nodesFlavor;
    DataFlavor[] flavors = new DataFlavor[1];
    DefaultMutableTreeNode[] nodesToRemove;

    public LinkFromTransferHandler() {
        try {
    ...
    public boolean canImport(TransferHandler.TransferSupport support) {
        ...
        try {
            ...
            for (DefaultMutableTreeNode node : nodes) {
                ...
                boolean found = false;
                for (String id : activeRule.getFromWhitelist()) {
    ...
    public boolean importData(TransferHandler.TransferSupport support) {
        ...
        try {
            ...
            for (DefaultMutableTreeNode node : nodes) {
                ...
                activeRule.getFromWhitelist().add(file.getRule().getId());
                activeRule.getFromBlacklist().remove(file.getRule().getId());
            }
